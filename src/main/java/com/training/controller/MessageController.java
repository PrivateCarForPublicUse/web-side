...
public class MessageController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name="tableName",value="对应的表名"),
            @ApiImplicitParam(name="idInTable",value="表内对应的id")
    })
    @ApiResponses({
            @ApiResponse(code=200,message = "成功获取反馈"),
            @ApiResponse(code=500,message="反馈不存在")
    })
    public ResponseResult findMessageByNameAndId(@RequestParam("tableName")String tableName, @RequestParam("idInTable")Long idInTable){
